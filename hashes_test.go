...
func TestHashSet(t *testing.T) {
	want := []uint64{211211211, 212211211, 211221211, 212221211}
	icon2 := images4.EmptyIcon()
	for i := range p2 {
		p2[i] = 255 * p2[i] // Premultiplication.
	}
	icon2.Pixels = p2
	got := HashSet9(icon2, 0.25, 4)
...
func TestCentralHash(t *testing.T) {
	want := uint64(333333333)
	icon1 := images4.EmptyIcon()
	for i := range p1 {
		p1[i] = 255 * p1[i] // Premultiplication.
	}
	icon1.Pixels = p1
	got := CentralHash9(icon1, 0.25, 4)
	if !reflect.DeepEqual(got, want) {
		t.Errorf("Wanted %v, got %v.", want, got)
	}

	want = uint64(211221211)
	icon2 := images4.EmptyIcon()
	icon2.Pixels = p2
	got = CentralHash9(icon2, 0.25, 4)
	if !reflect.DeepEqual(got, want) {
		t.Errorf("Wanted %v, got %v.", want, got)
	}
